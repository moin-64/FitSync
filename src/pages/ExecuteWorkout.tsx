...
import { Timer, Heart, Flame, Zap, BarChart } from 'lucide-react';
import { useToast } from '@/hooks/use-toast';
import { useUser } from '@/context/UserContext';
import { Workout, WorkoutHistory } from '@/types/user';
...
const ExecuteWorkout: React.FC = () => {
  ...
  const handleFinishWorkout = async () => {
    ...
    try {
      // Collect stats
      const performance = Math.floor(Math.random() * 30) + 70; // 70-100% performance
      const stats: Omit<WorkoutHistory, 'id' | 'workoutId' | 'date'> = {
      ...
      // Add missing properties that completeWorkout will set internally
      const completeStats = {
        ...stats,
        id: '', // Will be generated by completeWorkout
        workoutId: workout.id || '', // Pass the current workout ID
        date: new Date().toISOString() // Set current date
      } as WorkoutHistory;
      
      const success = await completeWorkout(workout.id || '', completeStats);
